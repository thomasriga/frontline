...
class BulletTower extends BaseTower {
	...
	public function BulletTower(x:Number, y:Number) {
		var depth:Number;
		...
		if(id >= 0) {
			...
			if(this.shot_delayC >= this.shot_delay) {
				...
				if(shootTheMF) {
					...
					bullet = _root.attachMovie("black_bullet", "bullet"+_root.getNextHighestDepth(), _root.app.actors.getNextDepth(Type.type_Bullets), {_x:this.base._x, _y:this.base._y,_width:6,_height:6});
					...
					bullet.onEnterFrame = function() {
						...
						for(x in _root.app.monsters) {
							if((not _root.app.monsters[x].isFlying()) and _root.app.monsters[x].doHitTest(this._x, this._y, true)) {
								_root.app.addMoney(
